          ...
          <>
            <div className="admin-grid">
              <AdminSection title="고객사" count={data.customers.length}>
                <form onSubmit={(event) => void save(event, "customers", (form) => ({ name: text(form, "name"), active: true }))}>
                  <label><span>고객사명</span><input name="name" required /></label>
                  <button className="primary-button" type="submit">고객사 추가</button>
                </form>
              </AdminSection>

              <AdminSection title="제품 / 설비" count={data.products.length}>
                <form onSubmit={(event) => void save(event, "products", (form) => ({ name: text(form, "name"), active: true }))}>
                  <label><span>제품 / 설비명</span><input name="name" required /></label>
                  <button className="primary-button" type="submit">제품 / 설비 추가</button>
                </form>
              </AdminSection>

              <AdminSection title="VOC 유형 / 세부 유형" count={data.voc_types.length}>
                <form onSubmit={(event) => void save(event, "voc_types", (form) => ({ voc_type: text(form, "voc_type"), voc_subtype: text(form, "voc_subtype"), active: true }))}>
                  <label><span>VOC 유형</span><input name="voc_type" required /></label>
                  <label><span>VOC 세부 유형</span><input name="voc_subtype" required /></label>
                  <button className="primary-button" type="submit">VOC 유형 추가</button>
                </form>
              </AdminSection>

              <AdminSection title="담당자 / 직책자" count={data.people.length}>
                <form onSubmit={(event) => void save(event, "people", (form) => ({ department: text(form, "department"), name: text(form, "name"), email: text(form, "email"), role: text(form, "role"), active: true }))}>
                  ...
                  <button className="primary-button" type="submit">담당자 추가</button>
                </form>
              </AdminSection>

              <AdminSection title="고정 최종 승인자" count={data.final_approver.length}>
                <form onSubmit={(event) => void save(event, "final_approver", (form) => ({ person_id: Number(text(form, "person_id")) }))}>
                  <label><span>고정 최종 승인자</span><select name="person_id" required><option value="">선택</option>{approvers.map((person) => <option key={String(person.id)} value={String(person.id)}>{String(person.department)} · {String(person.name)}</option>)}</select></label>
                  <button className="primary-button" type="submit">최종 승인자 저장</button>
                </form>
              </AdminSection>

              <AdminSection title="알림 템플릿" count={data.templates.length}>
                <form onSubmit={(event) => void save(event, "templates", (form) => ({ template_key: text(form, "template_key"), subject_template: text(form, "subject_template"), body_template: text(form, "body_template"), active: true }))}>
                  ...
                  <button className="primary-button" type="submit">템플릿 저장</button>
                </form>
              </AdminSection>

              <AdminSection title="알림 시각" count={data.notification_settings.length}>
                <form onSubmit={(event) => void save(event, "notification_settings", (form) => ({ weekday_time: text(form, "weekday_time"), timezone_name: "Asia/Seoul" }))}>
                  <label><span>평일 알림 시각</span><input name="weekday_time" type="time" defaultValue="09:00" required /></label>
                  <p>시간대: Asia/Seoul</p>
                  <button className="primary-button" type="submit">알림 시각 저장</button>
                </form>
              </AdminSection>
            </div>
